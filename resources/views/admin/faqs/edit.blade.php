    ...
    <form action="{{ route('admin.faqs.update', $faq->id) }}" method="POST">
        ...
        <div class="bg-gray-200/80 p-4 sm:p-5 rounded-xl shadow-sm border border-gray-300 mb-5">
            ...
            <input type="text" name="question"
                ...
                placeholder="Masukkan pertanyaan..." value="{{ old('question', $faq->question) }}">
        </div>
        ...
        <div class="bg-gray-200/80 p-4 sm:p-5 rounded-xl shadow-sm border border-gray-300 mb-5">
            ...
            <select name="status"
                class="w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#275931] focus:outline-none bg-white shadow-sm">
                <option value="published" {{ old('status', $faq->status) === 'published' ? 'selected' : '' }}>Publikasikan</option>
                <option value="not_published" {{ old('status', $faq->status) === 'not_published' ? 'selected' : '' }}>Tidak Publikasikan</option>
            </select>
        </div>
