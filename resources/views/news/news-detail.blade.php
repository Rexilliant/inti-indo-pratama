        ...
        <section class="mx-auto max-w-3xl px-4 sm:px-6 lg:max-w-screen-xl lg:p-4 lg:p-5 lg:w-full">            
            <article>
                ...
                <div
                    ...
                    <img src="{{ $news->getFirstMediaUrl('news-thumbnail') ?: 'https://placehold.co/1200x600/ECFDF5/047857?text=No+Image+Available' }}" alt="{{ $news->title }}"
                        class="w-full h-auto object-cover aspect-video hover:scale-[1.02] transition-transform duration-500">
                </div>

                {{-- Konten Artikel --}}
                <div class="text-base sm:text-lg text-gray-800 leading-relaxed space-y-5 sm:space-y-6 md:space-y-8 prose prose-lg prose-green max-w-none">
                    {!! $news->content !!}
                </div>
